...
|---|---|---|---|---|
| 1 | `OQ-11.3` | the **Phase 2 gate** | answered in intent (§1 + §5) — the human added protection | The "Main Protection" ruleset exists with `deletion`, `non_fast_forward` and `pull_request`, but its `ref_name.include` list is empty, so it targets **no branch**: `main` accepts a direct push today. Adding `~DEFAULT_BRANCH` (or `refs/heads/main`) to that list, and adding `bead-trailer` as a required check once the workflow exists, are repository settings only the human can make (`11` §5, `09`) |
